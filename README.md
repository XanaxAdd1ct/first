...
## Это высокозащищенный HTTP API сервер для централизованного управления доменными конфигурациями в распределенных сетях. Спроектирован для работы в условиях агрессивной среды, попыток перехвата трафика и DDoS-атак.
# Назначение
...
- Zero-Trust Architecture: Все данные, передаваемые агентам, подписаны HMAC-SHA256.
- Anti-Replay Protection: Система одноразовых токенов (Nonces) для Admin API.
- Security Monitoring: Встроенный детектор брутфорса и DDoS с автоматическим архивированием инцидентов.
- Resilience: Оптимистичная блокировка (WATCH/MULTI/EXEC) в Redis для исключения Race Conditions.
- Disk Protection: Логирование с автоматической ротацией (защита от переполнения диска при атаках).
- Modern TLS: Поддержка исключительно TLS 1.3 для максимальной криптостойкости.
...
- 
# 🔒 Security Deep Dive
1. Защита от Replay атак (Nonce)
Каждый запрос к Admin API требует уникальный заголовок X-Nonce.
- Механизм: Сервер запоминает каждый Nonce на 5 минут. Повторный запрос с тем же Nonce отклоняется (409 Conflict).
- Безопасность: Даже если атакующий перехватит валидный запрос с Bearer-токеном, он не сможет выполнить его повторно.

2. Мониторинг и архивирование инцидентов (security.go)
Система отслеживает подозрительную активность по IP:
- Брутфорс: >5 неудачных попыток авторизации в минуту.
- DDoS: >50 запросов в секунду (Rate Limit hits).
- Автоматизация: При накоплении 1000 событий инцидент упаковывается в .zip и отправляется в Telegram или сохраняется локально.
- Race-Free: Использование атомарных операций (sync/atomic) гарантирует точность подсчета под экстремальной нагрузкой.

3. Защита дискового пространства (logger.go)
Логгер пишет в logs.json с лимитом 100MB. При достижении лимита происходит ротация. Это предотвращает падение сервера из-за забитого диска при интенсивном логировании атак.

4. Hardened TLS
Конфигурация TLS 1.3 отсекает устаревшие уязвимые протоколы (SSL, TLS 1.0-1.2) и использует современные кривые X25519 и P256.
...
|---|---|---|
|ADMIN_TOKEN	|Да	|Bearer token (min 32 chars)|
|HMAC_SECRET	|Да	|Секрет для подписи payload|
|REDIS_URL	|Да	|URL Redis (redis://host:6379/0)|
|TLS_CERT_FILE	|Нет	|Путь к сертификату (включает HTTPS)|
|TLS_KEY_FILE	|Нет	|Путь к приватному ключу|
|TELEGRAM_TOKEN|	Нет	|Бот для алертов безопасности|
|TELEGRAM_CHAT_ID|	Нет|	ID чата для архивов инцидентов|
**ответ `200`**
```json
{
